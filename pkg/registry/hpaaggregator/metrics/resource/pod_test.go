	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			for _, partial := range []bool{true, false} {
				// setup
				r := NewPodTestStorage(tc.listerError, partial)
...
func NewPodTestStorage(listerError error, partial bool) *PodMetrics {
	return &PodMetrics{
		podLister: fakePodLister{data: createTestPods(), err: listerError, partialResult: partial},
...
func Test_NewPodMetrics(t *testing.T) {
	podLister := fakePodLister{
		data: createTestPods(),
	}
	getter := fakePodMetricsGetter{now: myClock.Now()}

	podMetrics := NewPodMetrics(schema.GroupResource{}, getter, podLister)
	...
	gotList := podMetrics.NewList()
	wantList := &metrics.PodMetricsList{}
	if !reflect.DeepEqual(gotList, wantList) {
		t.Errorf(`want = %+v, got: %+v`, wantList, gotList)
	}

	if podMetrics.Kind() != "PodMetrics" {
		t.Errorf(`want = NodeMetrics, got: %v`, podMetrics.Kind())
	}

	if !podMetrics.NamespaceScoped() {
		t.Errorf(`want = true, got: %v`, podMetrics.NamespaceScoped())
	}
