...
func Test_NewNodeMetrics(t *testing.T) {
	nodeLister := fakeNodeLister{
		data: createTestNodes(),
	}
	getter := fakeNodeMetricsGetter{now: myClock.Now()}

	nodeMetrics := NewNodeMetrics(schema.GroupResource{}, getter, nodeLister, nil)
	...
	gotList := nodeMetrics.NewList()
	wantList := &metrics.NodeMetricsList{}
	if !reflect.DeepEqual(gotList, wantList) {
		t.Errorf(`want = %+v, got: %+v`, wantList, gotList)
	}

	if nodeMetrics.Kind() != "NodeMetrics" {
		t.Errorf(`want = NodeMetrics, got: %v`, nodeMetrics.Kind())
	}

	if nodeMetrics.NamespaceScoped() {
		t.Errorf(`want = false, got: %v`, nodeMetrics.NamespaceScoped())
	}
