...
func ResourceLocation(restConfig *restclient.Config) (*url.URL, http.RoundTripper, error) {
...
func (r *ProxyREST) connectAllClusters(
	...
	proxyRequestInfo, err := r.resolver.NewRequestInfo(&http.Request{
	...
	// 1. for no resource request, proxy the request to kube apiserver.
	if !proxyRequestInfo.IsResourceRequest {
		return http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
			location := *r.admiralLocation
			location.Path = path.Join(location.Path, proxyPath)
			location.RawQuery = req.URL.RawQuery

			handler := proxy.NewThrottledUpgradeAwareProxyHandler(&location, r.admiralTransPort, true, false, responder)
			handler.ServeHTTP(rw, req)
		}), nil
	}

	clusterList, err := r.clusterLister()
	if err != nil {
		return nil, err
	}
	...
	if len(proxyRequestInfo.Name) != 0 {
		return r.requestWithResourceNameHandlerFunc(proxyPath, responder, clusterList), nil
	}
	return r.requestWithoutResourceNameHandlerFunc(proxyPath, responder, clusterList), nil
}

func (r *ProxyREST) requestWithResourceNameHandlerFunc(
	proxyPath string,
	responder rest.Responder,
	clusterList *apis.FederatedClusterList,
) http.Handler {
	...
	return http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		...
		if err != nil {
			responder.Error(errors.New("failed to renew RequestInfo for proxy"))
			return
		}
		...
		for i := range clusterList.Items {
			...
			go func(cluster *apis.FederatedCluster) {
				defer wg.Done()
				tlsConfig, err := proxy.GetTlsConfigForCluster(r.federatedInformerManager, cluster.Name)
				if err != nil {
					klog.Error(err)
					return
				}
				...
				impersonateToken, err := proxy.ImpersonateToken(r.federatedInformerManager, cluster.Name)
				if err != nil {
...
// nolint:gocyclo
func (r *ProxyREST) requestWithoutResourceNameHandlerFunc(
	proxyPath string,
	responder rest.Responder,
	clusterList *apis.FederatedClusterList,
) http.Handler {
