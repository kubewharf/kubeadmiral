...
// ConvertViaJson converts sourceObj to targetObj via json
func ConvertViaJson(sourceObj interface{}, targetObj interface{}) error {
	content, err := json.Marshal(sourceObj)
	if err != nil {
		return err
	}
	if err := json.Unmarshal(content, targetObj); err != nil {
		return err
	}
	return nil
}

func GetUnstructuredStatus(typedStatus interface{}) (map[string]interface{}, error) {
	unstructuredStatus := map[string]interface{}{}
	if err := ConvertViaJson(typedStatus, &unstructuredStatus); err != nil {
		return nil, err
	}
