...
func TestDeepHashObject(t *testing.T) {
	...
	for _, tc := range successCases {
		...
		for i := 0; i < 100; i++ {
			...
			if hash1a != hash1 {
				t.Errorf(
					"repeated hash of the same object (%q) produced different results: %d vs %d",
					toString(tc()),
					hash1,
					hash1a,
				)
			...
			if hash1a != hash2a {
				t.Errorf(
					"hash of the same object produced (%q) different results: %d vs %d",
					toString(tc()),
					hash1a,
					hash2a,
				)
...
func TestDeepObjectPointer(t *testing.T) {
	...
	myUni1 := unicycle{
		licencePlateID: "blah",
		primaryWheel:   &wheel1,
		tags:           map[string]string{"color": "blue", "name": "john"},
	}
	myUni2 := unicycle{
		licencePlateID: "blah",
		primaryWheel:   &wheel2,
	...
	for i := 0; i < 100; i++ {
		...
		if hash1 != hash3 {
			t.Errorf(
				"hash1 (%d) and hash3(%d) must be the same because although they point to different objects, they have the same values for wheel size",
				hash1,
				hash3,
			)
		}
