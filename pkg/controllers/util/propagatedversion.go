...
func ObjectNeedsUpdate(
	...
	needUpdate := true
	if desiredReplicas, err := utilunstructured.GetInt64Path(desiredObj, typeConfig.Spec.PathDefinition.ReplicasSpec, nil); err == nil {
		if currentReplicas, err := utilunstructured.GetInt64Path(clusterObj, typeConfig.Spec.PathDefinition.ReplicasSpec, nil); err == nil {
