...
var _ = ginkgo.Describe("Scheduling Profile", func() {
	...
	testPlacementFilter := func(ctx context.Context, profile *fedcorev1a1.SchedulingProfile, enabled bool) {
		...
		profile, err = f.HostFedClient().
			CoreV1alpha1().
			SchedulingProfiles().
			Create(ctx, profile, metav1.CreateOptions{})
		gomega.Expect(err).ToNot(gomega.HaveOccurred(), framework.MessageUnexpectedError)
		ginkgo.DeferCleanup(func(ctx ginkgo.SpecContext) {
			err := f.HostFedClient().
				CoreV1alpha1().
				SchedulingProfiles().
				Delete(ctx, profile.Name, metav1.DeleteOptions{})
		...
		configMap, err = f.HostKubeClient().
			CoreV1().
			ConfigMaps(f.TestNamespace().Name).
			Create(ctx, configMap, metav1.CreateOptions{})
		gomega.Expect(err).ToNot(gomega.HaveOccurred(), framework.MessageUnexpectedError)

		ginkgo.By("Verifying scheduling result")
		gomega.Eventually(func(g gomega.Gomega, ctx context.Context) {
			configMapFTC, exists := f.FTCManager().GetResourceFTC(corev1.SchemeGroupVersion.WithKind("ConfigMap"))
			g.Expect(exists).To(gomega.BeTrue())

			federatedConfigMapName := naming.GenerateFederatedObjectName(configMap.Name, configMapFTC.Name)
			federatedConfigMap, err := f.HostFedClient().
				CoreV1alpha1().FederatedObjects(f.TestNamespace().Name).
				Get(ctx, federatedConfigMapName, metav1.GetOptions{})
			gomega.Expect(err).To(gomega.Or(gomega.BeNil(), gomega.Satisfy(apierrors.IsNotFound)))
			g.Expect(err).ToNot(gomega.HaveOccurred())

			placement := federatedConfigMap.GetSpec().GetPlacementUnion()
			...
			if enabled {
				...
				for cluster := range placement {
					gomega.Expect(clusterSet.Has(cluster)).To(gomega.BeTrue())
				}
			}

			ginkgo.GinkgoLogr.Info("Obtained scheduling result", "result", placement.UnsortedList())
